...
@JBossLog
@ApplicationScoped
public class AlleleDTOConsumer implements Runnable {
    ...
    private String queueName = "alleleQueue";

    private final ScheduledExecutorService scheduler = Executors.newScheduledThreadPool(threadCount);

    void onStart(@Observes StartupEvent ev) {
        log.info("AlleleDTOConsumer Starting: " + threadCount + " threads Factory: " + connectionFactory1);
    ...
    public void run() {
        ...
        try {
            ...
            while (true) {
                ...
                allele = message.getBody(AlleleDTO.class);
                start = new Date();
                ...
                if((c % 200) == 0 && c > 0) {
                    ...
                    log.info("AlleleDTOConsumer " + rps + " r/s");
                }
                c++;
            }
        } catch (Exception e) {
            if(ctx != null) ctx.close();
            log.info("Failed Allele: " + allele);
    ...
    public void send(AlleleDTO allele) {
        producer.send(context.createQueue(queueName), context.createObjectMessage(allele));
    }
