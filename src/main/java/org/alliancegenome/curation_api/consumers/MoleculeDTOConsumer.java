...
@JBossLog
@ApplicationScoped
public class MoleculeDTOConsumer implements Runnable {
    ...
    private String queueName = "moleculeQueue";

    private final ScheduledExecutorService scheduler = Executors.newScheduledThreadPool(threadCount);

    void onStart(@Observes StartupEvent ev) {
        log.info("MoleculeDTOConsumer Starting: " + threadCount + " threads Factory: " + connectionFactory1);
    ...
    public void run() {
        ...
        try {
            ...
            while (true) {
                ...
                molecule = message.getBody(MoleculeDTO.class);
                start = new Date();
                ...
                if((c % 500) == 0 && c > 0) {
                    ...
                    log.info("MoleculeDTOConsumer " + rps + " r/s");
                }
                c++;
            }
        } catch (Exception e) {
            if(ctx != null) ctx.close();
            log.info("Failed Molecule: " + molecule);
    ...
    public void send(MoleculeDTO molecule) {
        producer.send(context.createQueue(queueName), context.createObjectMessage(molecule));
    }
