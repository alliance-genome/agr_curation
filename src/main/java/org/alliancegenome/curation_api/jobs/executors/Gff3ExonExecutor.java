...
public class Gff3ExonExecutor extends Gff3Executor {
	...
	private boolean runLoad(BulkLoadFileHistory history, List<String> gffHeaderData, List<ImmutablePair<Gff3DTO, Map<String, String>>> gffData, List<Long> idsAdded, BackendBulkDataProvider dataProvider) {
		...
		ph.startProcess("GFF Exon update for " + dataProvider.name(), (gffData.size() * 3) + 1);

		loadExonEntities(history, gffData, idsAdded, dataProvider, ph);
	...
	private void loadExonEntities(BulkLoadFileHistory history, List<ImmutablePair<Gff3DTO, Map<String, String>>> gffData, List<Long> idsAdded, BackendBulkDataProvider dataProvider, ProcessDisplayHelper ph) {
		...
		for (ImmutablePair<Gff3DTO, Map<String, String>> gff3EntryPair : gffData) {
			try {
				gff3Service.loadExonEntity(history, gff3EntryPair, idsAdded, dataProvider);
