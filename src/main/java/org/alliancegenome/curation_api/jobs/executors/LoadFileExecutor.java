...
public class LoadFileExecutor {
	...
	protected <S extends BaseEntityCrudService<?, ?>> void runCleanup(S service, BulkLoadFileHistory history, String dataProviderName, List<Long> annotationIdsBefore, List<Long> annotationIdsAfter, String loadTypeString) {
		runCleanup(service, history, dataProviderName, annotationIdsBefore, annotationIdsAfter, loadTypeString, false);
	}
	...
	protected void failLoad(BulkLoadFileHistory bulkLoadFileHistory, Exception e) {
		...
		// ObjectUpdateException carries its message in its data payload rather than in
		// the Throwable, so getMessage()/getLocalizedMessage() above are null for it.
		// Pull the real message(s) from the data so the failure isn't recorded as "null".
		...
		// Also record the failure in the exception list (bulkloadfileexception) so it
		// surfaces in the History exception table, not only as the errorMessage banner.
		ObjectUpdateExceptionData exceptionData = (e instanceof ObjectUpdateException oueWithData && oueWithData.getData() != null)
			? oueWithData.getData()
