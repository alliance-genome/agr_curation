...
@ApplicationScoped
public class ExpressionExecutor extends LoadFileExecutor {
	@Inject
	...
	public void execLoad(BulkLoadFile bulkLoadFile) {
		try {
			...
			runCleanup(geneExpressionAnnotationService, history, dataProvider.name(), annotationIdsBefore, annotationIdsLoaded, "expression annotation", bulkLoadFile.getMd5Sum());
			history.finishLoad();
