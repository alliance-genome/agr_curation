...
public class ConditionRelation extends UniqueIdAuditedObject {
	...
	@IndexingDependency(reindexOnUpdate = ReindexOnUpdate.SHALLOW)
	@ManyToMany
	@JsonView({CurationView.FieldsAndLists.class, CurationView.DiseaseAnnotation.class, CurationView.PhenotypeAnnotationView.class, CurationView.ConditionRelationView.class, CurationView.ForPublic.class, CurationView.ModelDocument.class})
	...
	public String getUniqueExperimentConditionId() {
		if (CollectionUtils.isEmpty(conditions) || conditionRelationType == null) {
			return "";
		}
		return conditions.stream()
			.map(experimentalCondition -> conditionRelationType.getName() + ":" + experimentalCondition.getConditionSummary()).collect(Collectors.joining(", "));
	}
