...
/**
 * SCRUM-6535: mirrors CassetteDTOValidator, minus the components a transgenic tool does not have.
 *
 * transgenic_tool_symbol_dto is not marked required in LinkML, unlike cassette_symbol_dto, so a
 * missing symbol is not an error here.
 */
@RequestScoped
public class TransgenicToolDTOValidator extends ReagentDTOValidator<TransgenicTool, TransgenicToolDTO> {

	@Inject TransgenicToolSymbolSlotAnnotationDTOValidator transgenicToolSymbolDtoValidator;
	@Inject TransgenicToolFullNameSlotAnnotationDTOValidator transgenicToolFullNameDtoValidator;
	@Inject TransgenicToolSynonymSlotAnnotationDTOValidator transgenicToolSynonymDtoValidator;
	@Inject TransgenicToolUseSlotAnnotationDTOValidator transgenicToolUseDtoValidator;
	...
	private List<TransgenicToolUseSlotAnnotation> validateTransgenicToolUses(TransgenicTool transgenicTool, TransgenicToolDTO dto) {
		...
		if (CollectionUtils.isNotEmpty(dto.getTransgenicToolUseDtos())) {
			for (int ix = 0; ix < dto.getTransgenicToolUseDtos().size(); ix++) {
				...
				ObjectResponse<TransgenicToolUseSlotAnnotation> useResponse = transgenicToolUseDtoValidator.validateTransgenicToolUseSlotAnnotationDTO(null, useDto);
				if (useResponse.hasErrors()) {
					allValid = false;
					response.addErrorMessages(field, ix, useResponse.getErrorMessages());
				} else {
					TransgenicToolUseSlotAnnotation use = useResponse.getEntity();
