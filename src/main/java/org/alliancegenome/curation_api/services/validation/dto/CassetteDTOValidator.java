...
import org.alliancegenome.curation_api.services.validation.dto.slotAnnotations.CassetteUseSlotAnnotationDTOValidator;
...
/**
 * SCRUM-6535: mirrors ConstructDTOValidator.
 *
 * The cassette associations are not validated here. They arrive through their own ingest sets and
 * files, so a cassette is created first and the associations resolve it afterwards by identifier.
 */
@RequestScoped
public class CassetteDTOValidator extends ReagentDTOValidator<Cassette, CassetteDTO> {

	@Inject CassetteSymbolSlotAnnotationDTOValidator cassetteSymbolDtoValidator;
	@Inject CassetteFullNameSlotAnnotationDTOValidator cassetteFullNameDtoValidator;
	@Inject CassetteSynonymSlotAnnotationDTOValidator cassetteSynonymDtoValidator;
	@Inject CassetteComponentSlotAnnotationDTOValidator cassetteComponentDtoValidator;
	@Inject CassetteUseSlotAnnotationDTOValidator cassetteUseDtoValidator;
	...
	public ObjectResponse<Cassette> validateCassetteDTO(CassetteDTO dto, BackendBulkDataProvider dataProvider) throws ValidationException {
		...
		List<CassetteUseSlotAnnotation> uses = validateCassetteUses(cassette, dto);
		if (cassette.getCassetteUses() != null) {
			cassette.getCassetteUses().clear();
		}
		if (uses != null) {
			if (cassette.getCassetteUses() == null) {
				cassette.setCassetteUses(new ArrayList<>());
			}
			cassette.getCassetteUses().addAll(uses);
		}
	...
	private List<CassetteUseSlotAnnotation> validateCassetteUses(Cassette cassette, CassetteDTO dto) {
		String field = "cassette_use_dtos";

		List<CassetteUseSlotAnnotation> validatedUses = new ArrayList<CassetteUseSlotAnnotation>();
		Boolean allValid = true;
		if (CollectionUtils.isNotEmpty(dto.getCassetteUseDtos())) {
			for (int ix = 0; ix < dto.getCassetteUseDtos().size(); ix++) {
				CassetteUseSlotAnnotationDTO useDto = dto.getCassetteUseDtos().get(ix);
				ObjectResponse<CassetteUseSlotAnnotation> useResponse = cassetteUseDtoValidator.validateCassetteUseSlotAnnotationDTO(null, useDto);
				if (useResponse.hasErrors()) {
					allValid = false;
					response.addErrorMessages(field, ix, useResponse.getErrorMessages());
				} else {
					CassetteUseSlotAnnotation use = useResponse.getEntity();
					use.setSingleCassette(cassette);
