...
export function SingleReferenceAdditionalFieldData({ fieldData }) {
	...
	} else if (fieldData['crossReferences'] && fieldData['crossReferences'].length > 0) {
		//if the crossReference is coming from rowData
...
const createCrossReferenceDivs = (resultList, crossReferences, field, shortCitation) => {
	...
	if (shortCitation)
		resultList.push(
			<div key={shortCitation} className="p-info">
				{shortCitation}
			</div>
		);
};
