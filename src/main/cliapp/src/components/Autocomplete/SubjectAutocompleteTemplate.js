	...
	if (item.geneSymbol) {
		...
		return (
			<div>
				<div
					onMouseOver={(event) => onSelectionOver(event, item, query, op, setAutocompleteHoverItem)}
					dangerouslySetInnerHTML={{ __html: item.alleleSymbol.displayText + ' (' + getIdentifier(item) + ') ' }}
				/>
			</div>
		);
		...
		return (
			<div>
				<div
					onMouseOver={(event) => onSelectionOver(event, item, query, op, setAutocompleteHoverItem)}
					dangerouslySetInnerHTML={{ __html: item.geneFullName.displayText + ' (' + getIdentifier(item) + ') ' }}
				/>
			</div>
		);
		...
		return (
			<div>
				<div
					onMouseOver={(event) => onSelectionOver(event, item, query, op, setAutocompleteHoverItem)}
					dangerouslySetInnerHTML={{ __html: item.alleleFullName.displayText + ' (' + getIdentifier(item) + ') ' }}
				/>
			</div>
		);
	} else if (item.name) {
		...
		return (
			<div>
				<div
					onMouseOver={(event) => onSelectionOver(event, item, query, op, setAutocompleteHoverItem)}
					dangerouslySetInnerHTML={{ __html: getIdentifier(item) }}
				/>
			</div>
		);
	}
