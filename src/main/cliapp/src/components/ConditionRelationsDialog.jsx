...
export const ConditionRelationsDialog = ({
	...
	const onRowEditCancel = (event) => {
		...
		if (originalConditionRelations && originalConditionRelations[event.index]) {
			...
			_localConditionRelations[event.index] = structuredClone(originalConditionRelations[event.index]);
					...
					toast_topright.current.show([
						{
							...
							detail: 'Could not update condition relation [' + localConditionRelations[event.index].id + ']',
							sticky: false,
						},
					]);
					reported = true;
		...
		if (originalConditionRelations && originalConditionRelations[index]) {
			...
			if (data.conditions.length !== originalConditionRelations[index].conditions.length) {
				...
				for (var i = 0; i < data.conditions.length; i++) {
					if (
						data.conditions[i].conditionSummary !== originalConditionRelations[index].conditions[i].conditionSummary
					) {
						rowsEdited.current++;
					}
				}
	...
	const validateRelation = async (relation) => {
		let _relation = structuredClone(relation);
	...
	const deleteAction = (props) => {
		return (
			<Button
				...
				onClick={(event) => {
					handleDeleteConditionRelation(event, props);
				}}
			/>
		);
	};
			...
			>
				...
				>
					...
					<Column
						...
						header="Relation"
						headerClassName="surface-0"
						body={conditionRelationTypeTemplate}
					/>
					<Column
