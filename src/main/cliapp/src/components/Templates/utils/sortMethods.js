...
export const crossReferencesSort = (crossReferences) => {
	return crossReferences.toSorted((a, b) =>
		a.displayName > b.displayName ? 1 : a.resourceDescriptorPage?.name === b.resourceDescriptorPage?.name ? 1 : -1
	);
};

export const diseaseQualifiersSort = (qualifiers) => qualifiers.toSorted((a, b) => (a.name > b.name ? 1 : -1));

export const evidenceCodesSort = (evidenceCodes) => {
	return evidenceCodes.toSorted((a, b) => (a.abbreviation > b.abbreviation ? 1 : a.curie === b.curie ? 1 : -1));
};
