		...
		if(processingEvent) {
			return (
				<div style={{ width: '20vw' }}>
					{ processingEvent.message }<br />
					Started: { new Date(processingEvent.startTime).toLocaleString() }<br />
					Update: { new Date(processingEvent.lastTime).toLocaleString() }<br />
			...
			return (
				<ProgressBar
					...
					displayValueTemplate={(value) => {
						return <>{rate}r/s -- {pe.currentCount}</>
					}}
				/>
			);
		} else {
