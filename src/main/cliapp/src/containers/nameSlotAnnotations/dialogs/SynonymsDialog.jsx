...
export const SynonymsDialog = ({
	...
	const onRowEditCancel = (event) => {
		...
		if (originalSynonyms && originalSynonyms[event.index]) {
			...
			_localSynonyms[event.index] = structuredClone(originalSynonyms[event.index]);
					...
					toast_topright.current.show([
						{
							...
							detail: 'Could not update ' + name + ' [' + localSynonyms[event.index].id + ']',
							sticky: false,
						},
					]);
					reported = true;
	...
	const saveDataHandler = () => {
		...
		mainRowProps.rowData[field] = localSynonyms;
		let updatedAnnotations = [...mainRowProps.props.value];
		updatedAnnotations[rowIndex][field] = localSynonyms;
	...
	const deleteAction = (props) => {
		return (
			<Button
				...
				onClick={(event) => {
					handleDeleteSynonym(event, props);
				}}
			/>
		);
	};
