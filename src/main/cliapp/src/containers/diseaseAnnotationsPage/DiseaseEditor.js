...
export const DiseaseEditor = ({ rowProps, searchService, autocompleteFields }) => {
  const [filteredDiseases, setFilteredDiseases] = useState([]);
  const [fieldValue, setFieldValue] = useState(rowProps.rowData.object.curie);
  ...
  const onDiseaseEditorValueChange = (event) => {
    ...
    if (event.target.value || event.target.value === '') {
      ...
      setFieldValue(updatedAnnotations[rowProps.rowIndex].object.curie);
    }
  };
