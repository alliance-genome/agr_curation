	...
	const onRowEditSave = (event) => {//possible to shrink?
		...
		mutation.mutate(updatedRow, {
			onSuccess: (data) => {
				...
				let conditionRelations = [...conditionRelations];
				conditionRelations[event.index] = data.data.entity;
				setConditionRelations(conditionRelations);
				const errorMessagesCopy = errorMessages;
				errorMessagesCopy[event.index] = {};
				setErrorMessages({...errorMessagesCopy});
			},
			onError: (error, variables, context) => {
				...
				setConditionRelations(conditionRelations);
				let _editingRows = {...editingRows, ...{[`${conditionRelations[event.index].id}`]: true}};
