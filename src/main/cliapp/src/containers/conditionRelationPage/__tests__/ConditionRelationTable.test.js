...
describe('<ConditionRelationPage />', () => {
	...
	it('Has the same text in edit mode', async () => {
		...
		expect(referenceAutocomplete.value).toEqual(
			'PMID:28806732 (ZFIN:ZDB-PUB-170815-1|DOI:10.1371/journal.pgen.1006959|PMCID:PMC5570503|AGRKB:101000000675992)'
		);
		expect(relationDropdown.previousSibling.firstChild.nodeValue).toEqual('has_condition');
		expect(experimentalConditionsAutocomplete.parentElement.previousSibling.firstChild.firstChild.nodeValue).toEqual(
			'standard conditions'
		);
	});
