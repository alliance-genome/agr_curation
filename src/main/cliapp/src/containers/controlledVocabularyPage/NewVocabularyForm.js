...
export const NewVocabularyForm = ({ newVocabularyDialog, setNewVocabularyDialog, setNewVocabulary }) => {
	...
	return (
		<div>
			...
			<Dialog
				visible={newVocabularyDialog}
				style={{ width: '450px' }}
				header="Add Vocabulary"
				modal
				className="p-fluid"
				footer={newVocabularyDialogFooter}
				onHide={hideVocabularyDialog}
			>
				<ErrorBoundary>
					<div className="field">
						...
						<InputText
							...
							required
							autoFocus
							className={classNames({ 'p-invalid': submitted && !vocabulary.name })}
						/>
					...
					<div className="field">
						...
						<InputText
							id="vocabularyLabel"
							value={vocabulary.vocabularyLabel}
							onChange={(e) => onChange(e, 'vocabularyLabel')}
							required
							autoFocus
							className={classNames({ 'p-invalid': submitted && !vocabulary.vocabularyLabel })}
						/>
						{submitted && !vocabulary.vocabularyLabel && (
							<small className="p-error">Vocabulary label is required.</small>
						)}
					</div>
					<div className="field">
						...
						<InputText
							id="vocabularyDescription"
							value={vocabulary.vocabularyDescription}
							onChange={(e) => onChange(e, 'vocabularyDescription')}
							required
							autoFocus
							className={classNames({ 'p-invalid': submitted && !vocabulary.vocabularyDescription })}
						/>
						{submitted && !vocabulary.vocabularyDescription && (
							<small className="p-error">Vocabulary description is required.</small>
						)}
					</div>
					<div className="field">
						...
						<Dropdown
							...
							className={classNames({
								'p-invalid': submitted && (vocabulary.obsolete === null || vocabulary.obsolete === undefined),
							})}
						/>
						{submitted && (vocabulary.obsolete === null || vocabulary.obsolete === undefined) && (
							<small className="p-error">Obsolete is required.</small>
						)}
					</div>
				</ErrorBoundary>
			</Dialog>
