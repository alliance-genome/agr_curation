...
describe("<GeneGeneticInteractionsPage />", () => {
	...
	it("The table contains correct data", async () => {
		...
		const interactorAGeneticPerturbationTd = await result.findByText("interactor A genetic perturbation (FB:FBal0196303)");
		const interactorBGeneticPerturbationTd = await result.findByText("interactor B genetic perturbation (FB:FBal0196304)");
