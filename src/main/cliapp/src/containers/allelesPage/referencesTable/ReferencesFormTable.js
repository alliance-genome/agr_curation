...
export const ReferencesFormTable = ({ references, editingRows, onRowEditChange, tableRef, deletionHandler }) => {
	return (
		<DataTable
			value={references}
			dataKey="crossReferencesFilter"
			showGridlines
			editMode="row"
			removableSort
			filterDisplay="row"
		...
		>
			<Column
				body={(data) => <DeleteAction deletionHandler={deletionHandler} id={data?.dataKey} />}
				header="Action"
				className="min-w-4rem max-w-4rem"
