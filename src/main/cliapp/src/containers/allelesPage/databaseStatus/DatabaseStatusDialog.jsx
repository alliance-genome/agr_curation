...
export const DatabaseStatusDialog = ({
	...
	const onRowEditCancel = (event) => {
		...
		if (originalDatabaseStatuses && originalDatabaseStatuses[event.index]) {
			...
			_localDatabaseStatuses[event.index] = structuredClone(originalDatabaseStatuses[event.index]);
	...
	const validateDatabaseStatus = async (gts) => {
		let _gts = structuredClone(gts);
		delete _gts.dataKey;
		const result = await validationService.validate(Endpoints.SlotAnnotation.ALLELE_DATABASE_STATUS, _gts);
		return result;
	};
