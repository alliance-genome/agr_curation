...
export const FunctionalImpactsDialog = ({
	...
	const onRowEditCancel = (event) => {
		...
		if (originalFunctionalImpacts && originalFunctionalImpacts[event.index]) {
			...
			_localFunctionalImpacts[event.index] = structuredClone(originalFunctionalImpacts[event.index]);
	...
	const compareChangesInFunctionalImpacts = (data, index) => {
		if (originalFunctionalImpacts && originalFunctionalImpacts[index]) {
			...
			if (
				...
				(!originalFunctionalImpacts[index].phenotypeTerm && data.phenotypeTerm) ||
				(originalFunctionalImpacts[index].phenotypeTerm &&
					data.phenotypeTerm &&
			...
			if (
				...
				(!originalFunctionalImpacts[index].phenotypeStatement && data.phenotypeStatement) ||
				(originalFunctionalImpacts[index].phenotypeStatement &&
					...
					toast_topright.current.show([
						{
							...
							detail: 'Could not update AlleleFunctionalImpact [' + localFunctionalImpacts[event.index].id + ']',
							sticky: false,
						},
					]);
					reported = true;
	...
	const functionalImpactsEditor = (props) => {
		return (
			<>
				...
				/>
				<DialogErrorMessageComponent errorMessages={errorMessages[props.rowIndex]} errorField={'functionalImpacts'} />
			</>
		);
	};
	...
	const deleteAction = (props) => {
		return (
			<Button
				...
				onClick={(event) => {
					handleDeleteFunctionalImpact(event, props);
				}}
			/>
		);
	};
		...
		<div>
			...
			>
				...
				>
					...
					/>
					<Column
						editor={functionalImpactsEditor}
