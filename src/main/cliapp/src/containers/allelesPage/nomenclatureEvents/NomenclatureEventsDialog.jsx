...
export const NomenclatureEventsDialog = ({
	...
	const onRowEditCancel = (event) => {
		...
		if (originalNomenclatureEvents && originalNomenclatureEvents[event.index]) {
			...
			_localNomenclatureEvents[event.index] = structuredClone(originalNomenclatureEvents[event.index]);
					...
					toast_topright.current.show([
						{
							...
							detail: 'Could not update AlleleNomenclatureEvent [' + localNomenclatureEvents[event.index].id + ']',
							sticky: false,
						},
					]);
					reported = true;
	...
	const deleteAction = (props) => {
		return (
			<Button
				...
				onClick={(event) => {
					handleDeleteNomenclatureEvent(event, props);
				}}
			/>
		);
	};
		...
		<div>
			...
			>
				...
				>
					...
					/>
					<Column
						editor={(props) => nomenclatureEventEditor(props)}
						field="nomenclatureEvent.name"
						header="Nomenclature Event"
						headerClassName="surface-0"
						body={nomenclatureEventTemplate}
					/>
					<Column
