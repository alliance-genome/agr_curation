			...
			<div className="layout-config-content">
				...
				<div className="config-scale">
					<Button
						...
						disabled={themeState?.scale === scales[0]}
					/>
					{scales.map((item) => {
						return (
							<i className={classNames('pi pi-circle-on', { 'scale-active': item === themeState?.scale })} key={item} />
						);
					})}
					<Button
						icon="pi pi-plus"
						...
						className="p-button-text"
						disabled={themeState?.scale === scales[scales.length - 1]}
					/>
				</div>
